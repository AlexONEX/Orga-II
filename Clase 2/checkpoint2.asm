...
sum_z:
	...
	xor rsi, rsi	; limpiamos rsi 
			; observar como tratamos de no usar no volátiles
			; caso contrario tendríamos que haber preservado mas registros en el stack 
...
product_2_f:
	...
	cvtsi2sd xmm1, rax	;convertimos el escalar rax en un double en xmm1. x1 convertido a double. 
	cvtss2sd xmm0, xmm0	;convertimos el float(32 bits) en xmm0 en un double(64 bits) en xmm0. Convierto f1 a 64.
	mulsd xmm0, xmm1	;multiplicamos los doubles xmm0 <- xmm0 * xmm1. f1*x1 = xmm0
	cvtsd2si rax, xmm0	;convetimos el double xmm0 en un escalar de 64 bits en rax. xmm0 = escalar. 
	mov [rdi], eax		;guardamos el resultado en la posición apuntada por rdi
				;des referencia de destination
				;¿qué problema hay con esta asignación? 
				;Puedo perder bits usando los 32 de 64 bits de rax. 	
	ret
